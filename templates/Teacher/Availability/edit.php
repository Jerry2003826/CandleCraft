            ...
            ?>
                <div class="slot-row" style="background-color: var(--admin-search-bg); border-radius: 12px; padding: 20px; margin-bottom: 16px; display: flex; align-items: flex-end; gap: 16px; flex-wrap: wrap;">
                    ...
                    <div style="padding-bottom: 2px;">
                        <button type="button" onclick="this.closest('.slot-row').remove()" class="admin-action-link delete" style="padding: 10px 16px; height: auto; width: auto;" title="Remove Slot">
...
function addSlot() {
    ...
    row.innerHTML = `
        <div class="flex-grow-1" style="min-width: 120px;">
            <label class="admin-form-label">Day</label>
            <select name="slots[${slotIndex}][day_of_week]" class="admin-form-select">
                ${options}
            </select>
        </div>
        <div class="flex-grow-1" style="min-width: 120px;">
            <label class="admin-form-label">From</label>
            <input type="time" name="slots[${slotIndex}][start_time]" value="09:00" class="admin-form-input">
        </div>
        <div class="flex-grow-1" style="min-width: 120px;">
            <label class="admin-form-label">To</label>
            <input type="time" name="slots[${slotIndex}][end_time]" value="17:00" class="admin-form-input">
        </div>
        <div style="padding-bottom: 2px;">
            <button type="button" onclick="this.closest('.slot-row').remove()" class="admin-action-link delete" style="padding: 10px 16px; height: auto; width: auto;" title="Remove Slot">
                <i class="bi bi-trash"></i>
