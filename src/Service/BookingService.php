...
class BookingService
{
    ...
    public function createBookingForStudent(int $classId, int $studentId, ?int $parentId = null, array $options = []): array
    {
        ...
        return $connection->transactional(function () use ($classId, $studentId, $parentId, $allowedClassStatuses): array {
            ...
            if (
                $this->enrollmentState->findBlockingBookingForStudent(
                    $classId,
                    $studentId,
            ...
            $activeCount = $this->enrollmentState->countBlockingBookingsForClass($classId, self::ACTIVE_BOOKING_STATUSES);
            ...
            if ($existingBooking) {
                if (
                    (string)$existingBooking->booking_status === 'cancelled'
                    || $this->enrollmentState->bookingCanBeReused($existingBooking)
                ) {
                    $now = DateTime::now();
                    ...
                    $existingBooking->reminder_sent_at = null;
                    $existingBooking->booking_confirmation_sent_at = null;
